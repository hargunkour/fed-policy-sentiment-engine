...
def _render_card(title: str, value, view_name: str):
    with st.container(border=True):
        ...
        with title_col:
            st.markdown(
                f'<div class="section-label" style="padding-top: 0.5rem;">{title}</div>',
        ...
        st.markdown(
            f'<div class="font-serif" style="text-align:center; font-size:2.4rem; '
            f'font-weight:700; margin: 0.5rem 0;">{value}</div>',
            unsafe_allow_html=True,
        )
